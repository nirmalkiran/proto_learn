...
serve(async (req) => {
  ...
  try {
    ...
    const { swaggerSpec, azureConfig, customPrompt, projectId } = body;
...
Please generate API test cases that include:
1. Positive test scenarios for each endpoint
2. Negative test scenarios (invalid inputs, unauthorized access)
...
Generate 8-12 comprehensive test cases covering the most important API scenarios and endpoints. Focus on quality over quantity.
