...
Format the response as a professional document with clear sections, bullet points, and actionable insights for stakeholders.`;
    ...
    // Support both baseURL and endpoint field names for compatibility
    const azureEndpoint = openAIConfig.baseURL || openAIConfig.endpoint;
    ...
    console.log('API Configuration:', {
      ...
      hasApiVersion: !!openAIConfig.apiVersion,
    ...
    const apiEndpoint = isAzure 
      ? `${azureEndpoint}/openai/deployments/${deploymentId}/chat/completions?api-version=${apiVersion}`
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        const errorMsg = isAzure 
          ? `Azure OpenAI authentication failed (${response.status}). Your API key may be expired or invalid. Endpoint: ${openAIConfig.baseURL}, Deployment: ${deploymentId}. Please update your Azure OpenAI credentials in the Integrations tab.`
