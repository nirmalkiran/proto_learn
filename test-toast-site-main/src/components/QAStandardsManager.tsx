  ...
  const content = (
    <div className="space-y-4">
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Enter rules as JSON or one rule per line. These rules guide AI generation.
                </p>
              </div>
              <div className="space-y-2">
                <Label>Examples (JSON or one per line)</Label>
                <Textarea
                  value={formExamples}
                  onChange={(e) => setFormExamples(e.target.value)}
                  placeholder={'Good: "Verify user can login with valid credentials"\nBad: "Login test"'}
                  rows={4}
                  className="font-mono text-sm"
                />
                <p className="text-xs text-muted-foreground">
                  Provide good/bad examples to help AI understand your standards.
                </p>
              </div>
              <div className="flex items-center gap-2">
                <Switch
                ...
                />
                <Label htmlFor="is-active">Active (AI will use this standard)</Label>
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowCreateDialog(false)}>Cancel</Button>
              <Button onClick={handleCreateStandard}>Create Standard</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-8">
            ...
            <p className="text-muted-foreground text-center">
              {standards.length === 0
                ? "No standards defined yet. Create your first standard to guide AI generation."
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge variant="outline">{getTypeLabel(standard.standard_type)}</Badge>
                      </div>
                      <p className="text-sm text-muted-foreground mb-2">
                        <span className="font-medium">Rules:</span> {getRulesPreview(standard.rules)}
                        {(Array.isArray(standard.rules) && standard.rules.length > 3) && "..."}
                      </p>
                      {standard.examples && standard.examples.length > 0 && (
                        <div className="flex items-center gap-1 text-sm text-muted-foreground">
                          <FileText className="h-3 w-3" />
                          <span>{standard.examples.length} example(s)</span>
                        </div>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => setStandardToDelete(standard.id)}
                      >
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Edit QA Standard</DialogTitle>
            <DialogDescription>
              Update the rules and examples for this standard.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-2 gap-4">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdateStandard}>Save Changes</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Delete Confirmation */}
      <AlertDialog open={!!standardToDelete} onOpenChange={() => setStandardToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => standardToDelete && handleDeleteStandard(standardToDelete)}
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle>QA Standards</CardTitle>
        </div>
        <CardDescription>
          Define rules and examples that guide AI when generating test cases and automation steps
        </CardDescription>
      </CardHeader>
      <CardContent>
        {content}
