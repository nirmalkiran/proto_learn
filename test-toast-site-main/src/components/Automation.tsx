...
export const Automation = ({ projectId }: AutomationProps) => {
  ...
  const [pendingAutomationCode, setPendingAutomationCode] = useState<string>('');
  const [pendingConfidence, setPendingConfidence] = useState(0.7);
  ...
  const generateAutomationCode = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (safetyConfig.requireApprovalForAutomation) {
          ...
          if (result.requiresApproval) {
            setPendingAutomationCode(code);
            setPendingConfidence(result.confidence);
            setPendingWarnings(result.warnings);
            setShowApprovalDialog(true);
            toast({
              title: "Review Required",
              description: "Automation code requires approval before use",
            });
          } else {
            setGeneratedCode(code);
            toast({
              title: "Success",
              description: "Automation framework generated and auto-approved",
            });
          }
        } else {
          setGeneratedCode(code);
  ...
  const handleApproveAutomation = async (approvedItems: any[]) => {
    setIsProcessingApproval(true);
    try {
      const code = approvedItems[0]?.content || pendingAutomationCode;
      setGeneratedCode(code);
      ...
      if (session?.user?.id) {
        await supabase.from('qa_ai_feedback').insert({
          ...
          action: 'approved',
          original_content: pendingAutomationCode,
          edited_content: code !== pendingAutomationCode ? code : null,
          feedback_notes: JSON.stringify({ confidence: pendingConfidence }),
        });
      }
      
      setShowApprovalDialog(false);
      setPendingAutomationCode('');
      toast({
        title: "Approved",
        description: "Automation code has been approved and is ready for use",
      });
    } catch (error) {
  ...
  const handleRejectAutomation = async (reason: string) => {
    try {
      // Log rejection
      const { data: { session } } = await supabase.auth.getSession();
      if (session?.user?.id) {
        await supabase.from('qa_ai_feedback').insert({
          ...
          action: 'rejected',
          original_content: pendingAutomationCode,
          feedback_notes: JSON.stringify({ confidence: pendingConfidence, reason }),
        });
      }
      ...
      toast({
        title: "Rejected",
        description: "Automation code has been rejected",
      });
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <AIContentApprovalDialog
        ...
        artifactType="automation_script"
        items={pendingAutomationCode ? [{ 
          title: `${programmingLanguage} Automation Script`,
          description: `Generated for ${userStories.find(us => us.id === selectedUserStory)?.title || 'User Story'}`,
          content: pendingAutomationCode 
        }] : []}
