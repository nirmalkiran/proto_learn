...
export const UserStories = ({ onViewChange, projectId }: UserStoriesProps) => {
  ...
  const { storeEmbedding, safetyControls } = useAILearning(projectId);
  const [stories, setStories] = useState<UserStory[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [isSyncing, setIsSyncing] = useState(false);
  const [generatingTestCases, setGeneratingTestCases] = useState<string | null>(null);
  
  // Approval dialog states for test cases from user stories
  const [showTestCaseApprovalDialog, setShowTestCaseApprovalDialog] = useState(false);
  const [pendingTestCasesFromStory, setPendingTestCasesFromStory] = useState<any[]>([]);
  const [pendingStoryId, setPendingStoryId] = useState<string | null>(null);
  const [pendingTestCaseConfidence, setPendingTestCaseConfidence] = useState(0.7);
  const [pendingTestCaseWarnings, setPendingTestCaseWarnings] = useState<string[]>([]);
  ...
  const [useLearnedPatterns, setUseLearnedPatterns] = useState(true);
  ...
  const [selectedStoryForDetails, setSelectedStoryForDetails] = useState<UserStory | null>(null);
  ...
  const generateTestCases = async (storyId: string, customPrompt?: string, imageFiles?: File[]) => {
    ...
    try {
      ...
      if (data.success && data.testCases) {
        ...
        if (safetyControls.safetyConfig.requireApprovalForTestCases) {
          const result = await safetyControls.processGenerationResult(
            projectId,
            'test_case',
            data.testCases,
            data.confidence || 0.7,
          ...
          if (result.requiresApproval) {
            setPendingTestCasesFromStory(data.testCases);
            setPendingStoryId(storyId);
            setPendingTestCaseConfidence(result.confidence);
            setPendingTestCaseWarnings(result.warnings);
            setShowTestCaseApprovalDialog(true);
            setGeneratingTestCases(null);
            toast({
              title: "Review Required",
              description: "Generated test cases require approval before saving",
            });
            return;
          }
        }

        // Auto-approved or approval not required - save directly
        ...
        const { data: insertedTestCases, error: insertError } = await supabase
          ...
          .select("id, title, description, steps, expected_result, priority");

        if (insertError) {
          throw insertError;
        }

        // Generate and store embeddings for each test case (fire and forget)
        if (insertedTestCases && projectId) {
          insertedTestCases.forEach((tc: any, index: number) => {
            const originalTestCase = data.testCases[index];
            const stepsText = originalTestCase.steps
              ? Array.isArray(originalTestCase.steps)
                ? originalTestCase.steps.join("\n")
                : originalTestCase.steps
              : "";
  ...
  return (
    <div className="space-y-6">
                            ...
                            <TableRow key={story.id}>
                              ...
                              <TableCell className="max-w-64">
                                <div 
                                  ...
                                  title={selectedStoryForDetails?.description?.replace(/<[^>]*>/g, '') || ''}
        ...
        <DialogContent className="sm:max-w-[600px]">
          ...
          <div className="space-y-4 py-4">
            <div className="flex items-center space-x-2">
              <Checkbox
                id="use-learned-patterns-initial"
                checked={useLearnedPatterns}
                onCheckedChange={(checked) => setUseLearnedPatterns(checked as boolean)}
              />
              <Label htmlFor="use-learned-patterns-initial" className="text-sm font-medium cursor-pointer">
                Use Learned Patterns
              </Label>
              <span className="text-xs text-muted-foreground">
                (Apply approved patterns from previous test cases)
              </span>
            </div>
      ...
      <Dialog open={showCustomPromptDialog} onOpenChange={setShowCustomPromptDialog}>
        <DialogContent className="sm:max-w-[600px]">
          ...
          <div className="space-y-4 py-4">
            <div className="flex items-center space-x-2">
              <Checkbox
                id="use-learned-patterns-regen"
                checked={useLearnedPatterns}
                onCheckedChange={(checked) => setUseLearnedPatterns(checked as boolean)}
              />
              <Label htmlFor="use-learned-patterns-regen" className="text-sm font-medium cursor-pointer">
                Use Learned Patterns
              </Label>
              <span className="text-xs text-muted-foreground">
                (Apply approved patterns from previous test cases)
      ...
      <Dialog open={showDetailsDialog} onOpenChange={setShowDetailsDialog}>
        <DialogContent className="sm:max-w-[700px] max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedStoryForDetails?.readable_id || 'N/A'} - Complete details of the user story
            </DialogDescription>
          </DialogHeader>
          {selectedStoryForDetails && (
            <div className="space-y-6 py-4">
              ...
              <div>
                ...
                <p className="mt-1 text-lg font-semibold">{selectedStoryForDetails.title}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="mt-1">
                    <Badge className={getPriorityColor(selectedStoryForDetails.priority)}>
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <Badge variant="secondary">{selectedStoryForDetails.status}</Badge>
                  </div>
                </div>
              </div>

              {/* Board and Sprint */}
              {(selectedStoryForDetails.boardName || selectedStoryForDetails.sprintName) && (
                <div className="grid grid-cols-2 gap-4">
                  {selectedStoryForDetails.boardName && (
                    <div>
                      <Label className="text-sm font-medium text-muted-foreground">Board</Label>
                      <p className="mt-1">{selectedStoryForDetails.boardName}</p>
                    </div>
                  )}
                  {selectedStoryForDetails.sprintName && (
                    <div>
                      <Label className="text-sm font-medium text-muted-foreground">Sprint</Label>
                      <p className="mt-1">{selectedStoryForDetails.sprintName}</p>
                    </div>
                  )}
                </div>
              )}
              ...
              <div>
                ...
                <p className="mt-1">
                  {selectedStoryForDetails.testCasesGenerated > 0 
                    ? `${selectedStoryForDetails.testCasesGenerated} test case(s) generated`
                ...
                <div className="mt-1 p-3 bg-muted/50 rounded-md">
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ 
                      __html: sanitizeHtml(selectedStoryForDetails.description) || "No description provided" 
                    }}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-1 p-3 bg-muted/50 rounded-md">
                  <div 
                    className="text-sm prose prose-sm max-w-none dark:prose-invert"
                    dangerouslySetInnerHTML={{ 
                      __html: sanitizeHtml(selectedStoryForDetails.acceptanceCriteria) || "No acceptance criteria provided" 
                    }}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-1">
                  <Badge variant="outline" className="capitalize">{selectedStoryForDetails.source}</Badge>
                </div>
              </div>
            </div>
          )}
          <DialogFooter>
            ...
            {selectedStoryForDetails && selectedStoryForDetails.testCasesGenerated === 0 && (
              <Button
                variant="gradient"
                onClick={() => {
                  setShowDetailsDialog(false);
                  handleGenerateClick(selectedStoryForDetails.id);
                }}
              >
                <Sparkles className="mr-2 h-4 w-4" />
                Generate Test Cases
      ...
      {/* Test Case Approval Dialog for User Stories */}
      <AIContentApprovalDialog
        open={showTestCaseApprovalDialog}
        onOpenChange={setShowTestCaseApprovalDialog}
        artifactType="test_case"
        items={pendingTestCasesFromStory.map((tc: any) => ({
        ...
        confidence={pendingTestCaseConfidence}
        warnings={pendingTestCaseWarnings}
        onApprove={async (approvedItems) => {
          if (!pendingStoryId || !projectId) return;
          const testCasesToInsert = approvedItems.map((tc: any) => ({
            project_id: projectId,
            user_story_id: pendingStoryId,
          ...
          await supabase.from("user_stories").update({ status: "completed" }).eq("id", pendingStoryId);
          setStories((prev) => prev.map((s) => s.id === pendingStoryId ? { ...s, testCasesGenerated: approvedItems.length, status: "completed" as const } : s));
          setShowTestCaseApprovalDialog(false);
          setPendingTestCasesFromStory([]);
          setPendingStoryId(null);
          toast({ title: "Approved", description: `${approvedItems.length} test cases saved` });
        }}
        onReject={(reason) => {
          setShowTestCaseApprovalDialog(false);
          setPendingTestCasesFromStory([]);
          setPendingStoryId(null);
          toast({ title: "Rejected", description: "Test cases were not saved" });
        }}
      />
    </div>
  );
