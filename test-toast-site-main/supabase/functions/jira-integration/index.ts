  ...
  try {
    ...
    if (action === 'get-sprints') {
      ...
      if (!sprintsResponse.ok) {
        ...
        // Handle boards that don't support sprints (e.g., Kanban boards)
        if (sprintsResponse.status === 400 && errorText.includes('does not support sprints')) {
          console.log('Board does not support sprints (likely a Kanban board), returning empty sprints array');
          return new Response(
            JSON.stringify({ 
              success: true, 
              sprints: [],
