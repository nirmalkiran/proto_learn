  ...
  setupServer() {
    ...
    this.app.get("/setup/status", async (req, res) => {
      try {
        ...
        res.json({ backend: true, agent: true, device: deviceStatus.connected ?? false, physicalDevice: physicalDeviceConnected, appium: appium.value?.running ?? false, emulator: emulator.value?.running ?? false });
      } catch { res.json({ backend: true, agent: true, device: false, physicalDevice: false, appium: false, emulator: false }); }
    });
