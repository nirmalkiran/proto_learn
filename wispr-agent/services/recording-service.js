...
class RecordingService extends EventEmitter {
  ...
  addStep(step) {
    ...
    // Deduplication logic: Check if the last step is basically identical
    const lastStep = this.recordedSteps[this.recordedSteps.length - 1];
    const now = Date.now();

    if (lastStep &&
      lastStep.type === step.type &&
      lastStep.locator === (step.locator || '') &&
      (now - lastStep.timestamp < 500)) {

      // Detailed check for coordinates or values
      let isDuplicate = false;
      if (step.type === 'tap' || step.type === 'longPress') {
        if (Math.abs(lastStep.coordinates?.x - step.coordinates?.x) < 5 &&
          Math.abs(lastStep.coordinates?.y - step.coordinates?.y) < 5) {
          isDuplicate = true;
        }
      } else if (lastStep.value === step.value) {
        isDuplicate = true;
      }

      if (isDuplicate) {
        console.log(`[RecordingService] Skipping duplicate step: ${step.type}`);
        return lastStep;
      }
    }
