...
import { Play, Square, Trash2, RefreshCw, Copy, Download, Monitor, Smartphone, Wifi, WifiOff, Save, GripVertical, Edit2, Check, X, ToggleLeft, ToggleRight } from "lucide-react";
      ...
      try {
        ...
        if (event.type && event.description) {
          const newAction: RecordedAction = {
            ...
            enabled: true, // New steps are enabled by default
          };
    ...
    try {
      ...
      if (data.success && data.steps) {
        const mappedSteps = data.steps.map((s: any) => ({
          ...
          coordinates: s.coordinates,
          timestamp: s.timestamp,
          enabled: s.enabled !== false, // Default to enabled
        }));
...
// Active steps: ${enabledActions.length} of ${actions.length}
                ...
                <ScrollArea className="h-[280px]">
                  {actions.map((a, i) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => setActions((prev) => prev.map((p) => p.id === a.id ? { ...p, enabled: !p.enabled } : p))}
                        className="pt-1"
                        title={a.enabled ? "Disable step" : "Enable step"}
                      >
                        {a.enabled ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`font-medium ${!a.enabled ? 'line-through' : ''}`}>
                          {i + 1}. {a.description}
                        </span>
                        {a.coordinates && (
                        ...
                        {editingLocatorId === a.id ? (
                          <div className="flex items-center gap-1 mt-1">
                            ...
                            <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => setEditingLocatorId(null)}>
                              <X className="h-3 w-3" />
                            </Button>
                          </div>
                        ) : (
                          <div className="flex items-center gap-1 group">
                            ...
                            <Button
                              ...
                              onClick={() => { setEditingLocatorId(a.id); setEditingLocatorValue(a.locator); }}
                            >
