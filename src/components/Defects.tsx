  ...
  const generateDefectReport = async () => {
    ...
    try {
      ...
      if (data.parsedReport) {
        ...
        setGeneratedReport({
          title: parsed.title,
          stepsToReproduce: Array.isArray(parsed.stepsToReproduce) ? parsed.stepsToReproduce : [parsed.stepsToReproduce || "Review scenario"],
          actualResult: parsed.actualResult,
          expectedResult: parsed.expectedResult,
          priority: parsed.priority || 'P3',
          severity: (parsed.severity === 'Medium' || parsed.severity === 'medium') ? '3 - Medium' : (parsed.severity || '3 - Medium'),
      ...
      // Store the generated report data for internal use
  ...
  const submitDefect = async () => {
    ...
    if (!azureDevOpsConfig || !azureDevOpsConfig.organizationUrl || !azureDevOpsConfig.projectName || !azureDevOpsConfig.personalAccessToken) {
      toast({
        ...
        description: "Please configure Azure DevOps in the Integrations tab first",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const newDefect: Defect = {
        id: data.workItemId?.toString() || Date.now().toString(),
      ...
      setShowAzureDefects(true); // Show Azure DevOps section again after submission

      toast({
        title: "Success",
        description: `Defect submitted successfully to Azure DevOps! Work Item ID: ${data.workItemId}`,
      });
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to submit defect to Azure DevOps",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
